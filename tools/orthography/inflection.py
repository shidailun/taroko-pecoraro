...
#
# [batch 201] That refusal is overturned, and the thing that overturned it is the
# same instrument this batch used on dmikat/nmikat: the prefixed slot is ruled
# because the bare name under it is ruled, not because the sound can be spelled.
# `diyan` is not the claim being made — `ndiyan -> ndiyan` claims only that the n-
# possessive of a name he capitalises is not a word any register will list, which
# is exactly what the block above already says of NDiyan. The identity entry that
# looked like a spelling verdict in 2026 reads, beside dMikat and dSbnawan, as a
# refusal to modernise a proper noun. Landing it took a second edit: the build log
# reported `curated keys that never landed [BLOCKED]: 1 — manual ndiyan->ndiyan`,
# because lexical_map.json carried a null for it, and a null outranks manual by
# design (the "stay green" pin). The pin came down with the refusal it enforced.
#
...
#
# [batch 201] dmikat, nmikat — `mikat` has been in this list since batch 144, and his
# own book writes the bare capitalised Mikat 9 times (訂婚 on DANGI, 未婚夫 on L'PONG,
# Mikatz ni Ingai on MALU). The two slots off it are the same d- / n- pair the Morisaka
# block above describes: § Wada stk'nai dMikat ka laqe mo 我女兒去和米卡特那群人作伴了,
# and § NMikat ka tn"tuan so lodoç 你剛切好的那隻雞是 Mikat 的. dSbnawan ni dTroko (batch
# 200) is the same d- over an ethnonym. No wordlist will ever list either one.
#
# [batch 201] ddliwis — the third turn of the same instrument, and the cheapest of
# the three: `liwis` is already in this list, so the only question ddLiwis raises is
# whether dd- over a hand-ruled name is still that name. § Ida bi msnoxel ka ddLiwis!
# 或許Liwis的朋友們在嫉妒吧！ — his own Chinese reads it as Liwis 的朋友們, the plural of
# the man. dd- is the same plural as dMikat and dSbnawan, doubled. Ruling it does not
# pay a sentence pair: its partner in that example is msnoxel, refused twice (b197,
# b199 — 嫉妒 is hkrig). A name is ruled because it is a name, not because a pair needs it.
...
daman mkefunang ddcristu put
boro saole lungat
dmikat nmikat pis ndiyan ddliwis
# [batch 202] The fifteen JAPANESE-ERA names on page 396, which fell between two
# stools. The generator excludes `name (.., jp)` from the tag seed on purpose --
# "whether Toro is turu or toro is a question about Japanese, not about his Truku
# spelling" -- and the loan population never picked them up either, so 15 of his
# 270 tagged names on that page were the only pale ones. The tag says NAME, so
# the class settles the colour; the ILRDF registry settles the spelling where it
# can. His own (m)/(f) tag is the second, independent test: a register form only
# counts if its 男名/女名 agrees with his.
#   RULED, shape and gender both agreeing: ako>aku 女, emi>emi 女 (exact),
#   toro>turu 男, kyosi>kiyusi 男 (the register writes the y-glide he elides),
#   natsö>natu 女, xeumi>humi 女 (his x>h, then the register).
#   REFUSED as spellings, kept as his own: miro (the register's miru is 男名 and
#   his MIRO is (f)), nuni, mati, dasi, doding, eeng, mori (morita is a Japanese
#   SURNAME, not this), xalo (x>g is not a rule; x>h is), tero (turu is already
#   TORO's, and two of his names cannot be one register name). For those nine the
#   char rules were also unpinned: o>u is a TRUKU rule and firing it on a Japanese
#   name is the very thing the generator's jp exclusion exists to prevent.
aku emi turu kiyusi natu humi
miro nuni mati dasi doding eeng mori xalo tero""".split()
...
# [batch 201] Four more the tagger cannot see, each named by the CHINESE of his own
# example and not by any tag: `suntyu` < 村長 sonchō (難道真的沒有人來款待我們的新村長嗎),
# `kaisia` < 会社 kaisha (在工廠工作，你中午要去哪裡吃飯), `bakuciku` < 爆竹 bakuchiku
# (所有孩子都愛放鞭炮), `armuni` < ハーモニカ (his MOBOX card is 吹奏 and the sentence is
# 吹口琴, two words long). Attestation is not a test any of these can sit.
# [batch 201] ega — 映画, eiga, the cinema. § Kmusa ko bi psinao daolyaq (= Mita ega)
...
# «Paaak», fait une très grosse pluie — is his spelling of a noise the rain makes, and no
# Truku wordlist will ever hold it. That is not a test the word fails; it is one it
# cannot sit. Kept separate from HAND_LOANS and HAND_NAMES because calling a rain sound
# a Japanese borrowing or a man's name would be false in the code's own vocabulary,
...
#
# The other six came in batch 203, on their own turn, and the seam closed at
# exactly the seven this comment priced. A token census of all 131 animal/plant
# cards -- per TOKEN, not per headword, because a multi-word head like WA"LO
# 蜜蜂 or "BAON 南瓜 has no single map key and the first census read those as
# green when walu and baun were already dark -- found 115 wholly dark and six
# with a pale or green token. Those six are these.
#
# Values, and what each rests on:
#   tyaqung  雉雞    his TYAQONG, o->u. Register has no 雉 at all.
#   pisuh    細腰蜂  his PISUX, x->h. Register's wasps are bnghur 土蜂 and
#                    srcing 虎頭蜂 -- different insects, so no substitution.
#   kjiyung  虎頭蜂  his KDIYONG. NOT a char rule: d+i -> j, which is Truku's
#                    own correspondence (jiyax, jirang) and which his book
#                    supports 11 dark to 0 -- the only two di- words that stay
#                    di- are the Japanese loans didensya and diku.
#   qmux     毒麥    his Q'MUX, identity. Register has no 麥 row.
#   jiram    斑鳩    his DILAM, and the shape is already ruled: dilas -> jiras
#                    is DARK, carrying d+i->j and l->r together, exactly this.
#   gaugan   荔枝    his GAOGAN, o->u.
# None of the six is attested, so none can freeze onto a homograph; and none of
# them blocks a sentence pair -- all six cards carry zero examples. This is a
# display ruling about his thematic lists, not a metric one.
HAND_SPECIES = """klulu tyaqung pisuh kjiyung qmux jiram gaugan""".split()

# AFFIX ARTICLES LONGER THAN A LETTER (batch 202). affix() in build_verified.py
# settles his twelve single-letter grammar cards -- an affix cannot be LISTED, but
# it can be attested as a PROCESS: how many modern types are this letter plus
# another modern type. His MPA card is the same kind of card ("Préfixe, parfois
# réalisé MP'") and fails only because the gate reads len(p) == 1.
#
# Measured the same way, over 32,208 modern types: `mpa` scores 0 -- but that is
# the wrong string to score, because his word-initial m before a labial is modern
# `em`, which is what the W tier already does 264 times. Score the MODERN prefix
# and `emp` returns 689, against a letter baseline of 16 (`i`) to 2,732 (`s`) and
# a three-letter noise floor of 1-8 (mda 8, mta 8, msa 5, mka 1).
#
# His own siblings had already ruled it and only the bare card was left behind:
# mpalaqe -> empalaqi and mpsiling -> empsiling are both DARK, the same prefix on
# the same page. A rule that fires on every sibling and not on the head is the
# batch-201 char-rule contradiction seen from the other side.
#
# The three sentences it holds are not word attestations either: one of them is
# him decomposing his own headword, § Mpaso (Mpa + So).
...
#
# Batch 201 (mtkkrang … qqrang) is the ONE-PAIR TAIL: after batch 200 exhausted the
# 2-or-more-pair tier, every remaining sole blocker frees exactly one sentence. 156
# types over 182 pairs, 109 of them never priced. These are the seven that ruled.
#   mtkkrang — and it sits in HAND_NOT_STACK six hundred lines below, which is not a
# contradiction. That entry refuses the PEEL: mt-k-krang reaches `krang` 碗掉下來破碎的
# 聲音, an onomatopoeion, and the peel had no business getting there. It says nothing
# about the word. His card is KK'LANG 發抖－打顫, `kkrang` and `mkkrang` are ruled dark
# on it (batch 191, keeping his reduplication against the register's krkran), and his
# sentence is 全身發抖. The mt- slot of that card is the same word as its k- slots.
# Refusing a mechanism is not refusing a form — this is the fix that ADDS.
#   qqrang < his QQ'LANG 顫抖, whose gloss line ends 參見 KK'LANG. He cross-references it
# himself, so this rides batch 191 exactly as `mkkrang` did, and the value it replaces
# (`qkrang`, from a generated rung) had a q AND a k, which no reduplication produces.
# It rests on a hand ruling rather than on attestation, and that is written down here
# so the next batch can see what it would be overturning.
#   npaamanu < empaamanu 能成什麼, LISTED and dark — his sentence is 對你們又有什麼用呢.
# The n- slot of an attested empaa-, same shape as batch 200's npaamalu.
#   npalaqi is a CONSISTENCY fix, not a new claim. His `npalaqe` was mapped `npaalaqi`
# with a doubled a while the dark sibling on his own LAQE card is `empalaqi` with one.
# Neither spelling is listed, so the card decides, and the card already spoke. Same
# shape as batch 200's npgxei→npghii: one slot out of step with its own paradigm.
#   kkslabang, tslbangi — both off LABANG 寬闊－過多, where `mslabang` 已經寬 is listed and
# dark and batch 199 already ruled `tslabang` on `labang` 寬 for his 逾越界限者. The
# greed reading (貪心, 覬覦) is his own extension of 過多 and 199 accepted it on this very
# card; these are the kk- and the -i slot of a stem the card has settled.
#   rrulung < his `llolong` 雲. `rulung`'s only gloss row reads 人名（男;女）— and batch
# 200's rule is that a single row is not the register's answer: `mrulung` 有雲 and
# `smrulung` 起雲 are the family, and his sentence is 是這會兒的雲把它遮暗了. His doubled
# onset is kept rather than replaced, the same choice batch 191 made for `kkrang`.
#   bbuway — SIX of the seven spellings on his BOAI 給 card already render dark: buway,
# embuway, kmbuway, embbuway, pbuway, sbgay. Only the bare reduplicated slot was pale,
# and it was pale rendering HIS OWN letters (`bboai`, a tier-M identity with no written
# reasoning behind it). `embbuway` carries the same `bb` on the same card. A pale slot
# beside six dark ones is the cheapest question on the page, and this card answered it.
#   mqqrang — the m- slot of the qqrang ruled four lines up, on his own QQ'LANG card,
# sentence 所有的孩子都嚇得發抖. Exactly how `mkkrang` rides `kkrang`.
#   qmilaw — his QNILAO card head is `qnilaw`, LISTED and dark and glossed 煮爛的食物,
# with `smqnilaw` 很需要豬食 and `tmqnilaw` 煮豬食的人 beside it: the register's word for
# pig-feed is his word. `qmilaw` is the regular <m> counterpart of that <n>, and his
# sentence 現在我要準備豬的飼料 is the actor focus the <m> marks.
#   wwahir < his `wwaxel`, and `wahir` 藤蔓（植物名）is LISTED and dark. His sentence is
# 好沿路砍（打）藤蔓和枝條 — the register's gloss is the sentence's own word. Doubled
# onset kept, like rrulung.
#   spsdharun is the SECOND form in this batch that HAND_NOT_STACK refuses and this list
# rules, and for the same reason. The peel reaches `hari` 一點（比較級）through a substring
# accident and deserved its refusal. But his SDAXAL card is `sdahar` 依靠著… — LISTED,
# dark, with `empsdahar` and `psdahar` dark on the card — and his sentence is 讓我們的祖父
# 靠一下. The syncope in the rendered form is HIS (he writes `dxal`, not `daxal`); the
# accident was in how the analyser got there, not in the word.
#   rrungay < his `llongai` 猴子們, and `rungay`'s only gloss row is 人名（男）— the second
# time in one batch a single row calls a common noun a personal name (rulung was the
# first). `maarungay` 像猴子 and `tmrungay` 專獵猴子 are the family, and `rungay`/`mrungay`
# are already dark on his own LONGAI 猴子 card. Third doubled onset kept, after rrulung
# and wwahir; his sentence is plural (猴子們到底都跑哪去了).
#   syukay is HIS OWN BOOK again. It looked bare and unsupported on the LUBWI card — and
# then the count came in: he writes `Ksyukai` twice on a KSIYUK card glossed 借入—借出,
# and that renders `ksyukay`, LISTED and dark and glossed 要借. Same spelling minus the
# k-, same gloss, his own hand. The -ay projective his sentence needs (讓我借兩袋你的花生)
# is the slot the register spells.
#   mnkpitu — `mkpitu` 第七 is LISTED and dark, and `empitu`/`kmpitu` are dark on his own
# PITO 七 card. The value it replaces, `mnpitu`, had DROPPED the k that every other slot
# on the card carries. Third out-of-step slot this batch, after npalaqi and spsdharun:
# the paradigm is the cheapest question on the page and it keeps answering.
#   gnaliq < his `gn'lyeq`, which was rendering `gneeliq` — a shape nothing lists. The
# register lists `gnaliq` 取過首級 and `gmaliq` 出草（殺人）, `gmaliq` is dark on this very
# card, and `gmnaliq` was hand-ruled in batch 197 off the same root. His sentence is a
# beheading. The gloss of the candidate IS the gloss of the entry it renders in.
#   empkduriq is the THIRD HAND_NOT_STACK form this batch, and the ruling deliberately
# does NOT follow the shortest path. His QDOLYAQ 逃跑 card carries `mpqdolyaq` AND
# `mpkdolyaq` as two distinct slots; sending both to the attested `empqduriq` would
# collapse a distinction he wrote. `qduriq` 逃跑 and `empqduriq` 要逃跑 are both listed
# and dark, so the card settles the stem, and the emp- normalisation is what his own
# sibling slot shows. His k stays. Decide slot by slot when a homophone exists.
#   sneuwit — `uwit` 疲累 is LISTED and dark, and the `eu` of the rendered stem is not a
# guess: `skeuwit` 因疲勞 and `pkeuwit` 使其疲累 both carry it. His sentence is 真累人.
#   nbliqan is a paradigm slot, nothing more. On his BLAEQ 享受——幸福 card `mblyeqan` →
# `embliqan` and `knblyeqan` → `knbliqan` are BOTH dark, and the register has `qbliqan`
# 財富 and `sbliqi` 幸福. Only the n- slot was pale.
#   knnlaan is a CONSISTENCY fix pointing the other way — his own letters, kept. The map
# had crossed l→r to `knnraan` while every other member of the family keeps the l and
# is dark: `snlaan` (LISTED), `gnlaan`, `smnlaan`. His 你的善良 is the GALU 溫柔、憐憫、
# 關愛 root, where `gnlaan` is glossed 因他曾對我們所懷的愛. A tier-M identity here is not
# a no-op — it is what blocks the char rule that broke the paradigm.
#   gneuwit is the gn- slot of the sneuwit two lines up, same WIT 疲勞－虛弱 card, with
# `keuwit` already dark on it. His 是靠他自己的辛勞得來的 is what a gn- instrumental says.
#   kkrut is the tail of a HOMOGRAPH FREEZE this batch found by reading his own book.
# He has TWO cards a fold apart — `"LÖT` 梁－橫梁－柱子 and `Q'LöT` 鋸子 — and the two
# elision marks are the only thing separating them, exactly the case the orthography
# rules warn about. Nine slots of the saw card already render krut-: kmrut 手術 (his own
# sentence there is 讓人開刀切開身體), empkrut, pkrut, skrut, mskrut, empskrut, pskrut. Only
# the bare head went elsewhere — `q'löt` was frozen on `qrut`, which is LISTED and DARK
# and glossed 啃骨聲, the sound of gnawing a bone. Dark, six occurrences, and about a
# different word entirely; the batch 171 DLUT shape. `krut` 切 / `mkrut` 鋸 is the word,
# so `q'löt`→`krut` and his reduplicated instrumental `qq'löt`→`kkrut`, doubled onset
# kept as in kkrang. The head moves dark-to-dark: the metric does not notice this fix,
# and it is the more important half of the slice.
#   tluk — his BTULUK 打——不用器具地敲打 card, where the b was never his own settled
# letter: his first example writes `Tlukan (btlukan ? - ptlukan ?)`, three spellings of
# one word with the question marks in his hand. The map had already dropped the b twice
# (`bntuluk`→pntuluk, `pnbtlukan`→pntlukan, both dark), `tlukan` and `tlukun` are dark,
# `ptuluk` 使打人 is dark, and `tluki` 去打 is LISTED. Only the b- slots were pale, and
# they were pale rendering his letters. Ruled the stem; btlukan/btlukun/btluki/pnbtuluk
# follow it in manual_map.
#   kmalux, mmalux — the SECOND homograph freeze this slice, same shape as kkrut. Batch
# 197 found `mnalux` 生病 by meaning and ruled `empkmalux` on it. All six of his `malox`
# occurrences are illness: 不舒服, 感冒, 痛苦, 弄病, 兩肩馬上就痛, 咳嗽. Yet `mmalox` was
# frozen on `mmlux` — LISTED, dark, glossed 很安靜, four occurrences of a sick man rendered
# as a quiet one. `kmalox` had gone the other way and taken the x→h char rule to `kmaluh`,
# which nothing lists. Both now render the stem `empkmalux` already carries.
#   npnalu is the n- slot of his NALU 代替、頂替 card, where `mnalu` is already hand-ruled
# and dark. 我會派兩個人代替我去. Same shape as npaamalu (b200) and npaamanu.
#   Two more this slice went into manual_map on HIS OWN BOOK rather than on a paradigm.
# `mulu` looked like a word — until the count came in: he writes the bare token `malu`
# 173 times and `mulu` exactly once, in a sentence glossed 沙質地最適合種花生. `malu` 好 is
# what 最適合 says. Same instrument as naqex×98 / Naqox×1 in batch 200.
# `mg'gui (ngagwi)` is his own parenthetical over a card whose `negeeguy` 應該偷偷 is
# already dark; `geeguy` 偷偷的拿東西 and `mgeeguy` are both LISTED, and his sentence is
# 當小偷. The dark side passes the gloss test here, which is what batch 200 refused for
# pgagu on this same page.
#   pneemuun, and a whole family respelled behind it. His "MU 極細的粉－細塵 card had its
# HEAD dark and every slot off it pale, which is the npalaqi shape one card larger: `'mu`
# renders `emu` (LISTED, dark) while pn'mu, mpn'mu, m'mu, n'mu, nn'mu and pnmuon all kept
# his bare `mu`. The register writes the stem `eemu` under every prefix it has — empeemu
# 要做是…粉, teemuun 要做糕餅, geemu, keemu, emeemu — so all six now carry it. Only the
# blocker is RULED: `pneemuun` has `teemuun` 要做糕餅 as its exact template, prefix+eemu+un,
# and his sentence is 把所有的燧石都磨成粉. `emu` glossed alone reads 香蕉糕；糖果, which
# would have refused this; the family reads 粉, and the family is the register's answer.
# The other five were respelled expecting them to stay pale, and all five went dark on
# their own account once the stem was right — the analyser could reach `emu` through
# `eemu` and never could through `mu`. Six gained where one was priced; the pale was
# a spelling defect on this card, not an evidence gap.
#   npamuxul rests on `uxul` 暖和（溫暖）, LISTED and dark, against his 這樣就會很暖和. The
# affix is his own (n-pa-m-), the same license npaamalu / npaamanu / npnalu ran on. Noted
# as the batch's least comfortable ruling: the register writes muuxul and meuxul with a
# linking vowel his -muxul has not got, and only the affix, not the root, is at issue.
#   empneemuun — his own parenthetical again, on the "MU card, and the pale side is the
# one carrying a suffix. He writes § Nn"mu so d'xgal ka iso ni mpn"mu so (mpnmuon so)
# dole 你來自塵土，也將歸於塵土 — two spellings offered for one slot, of which `mpn"mu`
# now renders `empneemu` (dark this batch, on the family gloss 粉) and `mpnmuon`
# rendered `empnmuun`, pale. The batch 200 instrument says the pale side should render
# what the dark side renders; here it can do better than that, because `pneemuun` is
# ALREADY ruled dark on this same card off his `pnmuon`. So `empneemuun` is em- over a
# slot the card has already settled, not a merge of two of his written forms, and his
# -un survives. Prefer the fix that adds.
#   qdrxun — his 我要沿著四周築一道石牆. `qdrux` 石牆 is LISTED and dark, and so is
# `qndrxan` 被砌的石牆, the syncopated perfective of the very same stem. The -un slot
# between them was the only pale one on the paradigm: the cheapest question on the page,
# and the card answers it.
#   ptryaun — his TLIA (TLIYA) 輪子－旋轉之物－旋轉 card, § Ana naqex ka tdoloi so o,
# ida mo ptlyaon sayang 雖然你的車拋錨了，我馬上讓它轉起來. `triya` is LISTED and dark
# and glossed 陀螺 — a spinning top, which is his 旋轉之物 word for word — with `mstriya`
# 旋轉 and `striya` 為…旋轉 beside it, and his own `mtriya` already dark on that root.
# The neighbour is dark for the right reason, so the p-…-un causative slot between them
# is the one his sentence spells: 讓它轉起來 is causing a thing to spin.
#   pstutuy, pskani — two more of his parentheticals, and both land on values the map
# already paints dark. § Ya bi psttui (= psttuy ?) ka pusu kari 別再提起我們爭論的話題:
# his whole TUTWI family renders `tutuy` (LISTED, 起來（從坐的姿勢起身）) — tutwi, mtutwi,
# mntutwi, pstutwi, pnstutwi, ktutwi, ptutwi, seven forms dark — and only these two
# syncopated spellings went elsewhere, `psttui` onto `ttui` 切、剁. A sixth homograph
# freeze, chopping instead of standing up; `pstutuy` 使扶起身 is the word, and 提起 a
# topic is exactly its causative. And on the UMUL card, § Asi na bsqani ka xea 他得把
# 它嚼碎: he writes `Ps"qan (= Psqan ? = Bsqan ?) (= R. "QAN ?)` on his "QAN 吃 card, so
# `bsqan` is his own third spelling of `psqan`, which the map already sends to `pskan`
# 咀嚼. The imperative follows it. His parenthetical is testimony; the map only had to
# be told the third spelling exists.
#   niyak — his YAQ card, 田裡除粗重工作以外的活，例如：除草、鋤地、培土, with 參見
# MIYAQ、IYAQ in his own hand. `miyak` is LISTED and already dark ON THIS CARD — the
# register calls it the root of empmiyak 要忙家務事 / mmiyak 忙家務事 — so the stem is
# settled and only the n- perfective slot was pale. § Wana musa niyaq tityex ska
# kmpaxan beilox 我在豆田裡只剩下一些零星的活了: the odd jobs that are left. The gloss
# says 家務 where he says 田裡, and that is worth writing down rather than hiding; but
# both are the routine chores one does, not two unrelated senses, so this is a paradigm
# question and not a homograph. The dark neighbour is dark for the right reason.
#   empiyax — a pale-to-pale consistency fix, and it earns its line because the char
# rule was contradicting the card. His YAX card is 兩個界限之間的（時間與空間的）間隔 and
# `iyax` 中間 is LISTED and dark on it, x intact; the register even holds `empaaiyax`
# 要成為間隔. Only his § qx'nu ka mp'yax so kmblagan 你留多大的間隔 took x→h and came
# out `mpyah`, an h beside its own root's x. Same shape as the l→r crossing batch 171
# found on the DLUT card.
#   pthnguli — his XUNGUL 鋒利的－銳利的 card, § Ptxnguli tityex ka "ma poting so 把你
# 刀子的刀刃磨一磨. Five slots on that card are already dark — hungul 刀鋒, khungul,
# mhungul, pthungul, mkan — and the register's pthungul- family is sharpening end to
# end (dmpthungul 只磨鋒利…的人, spthungul 請…削尖), so the stem is dark for the right
# reason. It even holds the syncopated imperative `phngli` 要削成尖, which is his
# sentence in the register's own words. His spelling keeps the pt- and the syncope, so
# the imperative slot is ruled on his letters rather than respelled onto phngli: prefer
# the fix that adds.
#   dpnaah, miq — the last two of the 1-pair tail that were GREEN rather than pale, so
# blk200.py reported them as absences and the DOM had to be asked instead. `pnaah` 自；從
# is LISTED and dark, and his § Psqlasao ta tityex ka dpnax isil alang 我們稍微招待一下
# 從外地來的人 puts his d- plural-of-persons over it, the same d- as dMikat and dSbnawan.
# And on his EQ card — 是！尤用於回應命令 — the head is already dark as `iq` 好(指沒問題),
# an e→i his own card established; § Ana manu ida m'eq ka xea 無論什麼事，他總是說『是』
# is the only other slot on a two-slot card, so the paradigm answers it.
#   empskla — his § Asi ka ttalang so bi, upsk'la so kia ka tdoloi 你要跑很快才能趕上車,
# which the char rules were rendering `upskra`: l→r fired on a root that keeps its l.
# `skla` is LISTED and dark, `empskla` 會趕上 is LISTED with his sentence for a gloss, and
# `msskla` 趕上 / `pskla` 使趕上 stand beside them. The third char-rule contradiction this
# batch, after mp'yax and the DLUT crossing.
#   The rain family — koyoç, npkoyoç, knkmoyoç, nkmoyoç — is not in this list; it is
# four manual_map entries onto quyux / npquyux / knqmuyux. It is recorded here because
# it is the seventh homograph freeze of batch 201 and the most instructive. His KOYOç
# card is 雨, and the map was sending its head to `kuyuh` 女人: dark, and wrong, on a
# word whose shape it happens to share. The fix looked like a five-key sweep until the
# gained/LOST check reported `LOST ['empakuyuh', 'npkuyuh']` — one of those was the
# freeze coming off, the other was his OTHER card breaking. He writes KOYOX 女人—妻子—
# 雌性 with x and KOYOç 雨 with ç, and § Mpakoyox so ka yako 即使你說不，你也一定會成為我
# 的妻子 belongs to the first. `koyox -> quyux` was removed and only the ç-side kept.

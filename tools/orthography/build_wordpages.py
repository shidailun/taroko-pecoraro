...
none are emitted:
  ...
  group 2 — two or more candidate roots, or a single one that is not dark.
            ...
            Emitted as `"?"`: the refusal was only ever about NAMING A ROOT, and
            it had been withholding the page as well. A concordance asserts
            nothing about morphology, so there is nothing in a list of his own
            sentences that an unresolved analysis makes unsafe. Withholding it
            left `qmpahan` (58 occurrences) and `pngalun` unreachable, and made
            a search for either haul in whole root entries instead.

The file is a TABLE, not a renderer. `""` means group 3, `"?"` means group 2, and
any other value is the modern spelling of the group-1 root. The two silent values
are kept apart because their cards must not say the same thing: group 3's says no
analysis reaches a root, which for group 2 is false. Reachability is re-tested by
...
def main():
    ...
    for t in sorted(set(used) - indexed):
        ...
        if not cands:
            ...
            out[t] = "?"                      # group 2
            counts["2 ambiguous root, page only"] += 1
    ...
    f.write(
        ...
        u"// single candidate root is ALREADY dark (the value is that root),\n"
        u"// %d that no affix analysis reaches at all (the value is empty), and\n"
        u"// %d the analysis reaches by more than one route (the value is \"?\";\n"
        u"// the card names no root and carries only the concordance).\n"
        u"// app.js builds a page only for a key this file emits.\n"
        % (len(out), counts["1 one dark root"], counts["3 concordance only"],
           counts["2 ambiguous root, page only"]))

...
`tools/orthography/build_modern_map.py`, 6,733 tokens of which 4,732 actually
...
- **id** (1,088) — original spelling already attested in the omnibus; left unchanged
...
- **M** (692) — hand-curated, gloss-verified (`tools/orthography/manual_map.json`).
...
- **T** (234) — sister-dialect triangulation: Toda/Tgdaya cognates VALIDATE which
...
- **P** (1,136) — root-consistency projection: a resolved family member fixes the
...
- **R** (697) — relative inheritance. The other tiers test WHOLE words against the
...
- **E** (165) — projection into his own example sentences. Tier P refuses example
...
- **G** (30) — root projection ACROSS entries. Tier E only sees the entry a token
...
- **W** (140) — the written schwa before a word-initial labial. A post-pass, run after
  V: it corrects values the other tiers already produced rather than reaching a token
  nothing else did, which is why 99 of its 140 come from P and only 6 from M. Modern
  ...
  per token — his form unattested in **both** corpora, the e-form attested — so the
  class evidence never overrides evidence about the word in hand. Log:
  `tier_w_log.txt`.

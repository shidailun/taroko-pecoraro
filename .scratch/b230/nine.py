...
for w in WORDS:
    ...
    # roots() is a METHOD on class Inflection, constructed with (lex, MODERN_MAP)
    try:
        r = INF.roots(w)
        print("  roots(%s): %s" % (w, r[:4] if r else "level 0 -- no analysis"))
        for cand in sorted(set(x[0] for x in (r or []))):
            print("    cand %-10s listed=%-5s %s" % (cand, cand in AM, gl(cand)))
